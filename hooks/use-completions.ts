...
export function useCompletions(date?: string, timezone?: string) {
  ...
  const queryDate =
    date || (timezone ? getTodayInTimeZone(timezone) : undefined)
  ...
  const {
    ...
    staleTime:
      queryDate === getTodayInTimeZone(timezone || "UTC")
        ? 1000 * 60
        : 1000 * 60 * 10,
    enabled: !!queryDate,
  })
  ...
  const toggleMutation = useMutation({
    ...
    onMutate: async ({ habitId, date, completed }) => {
      ...
      const previous = queryClient.getQueryData<CompletionRow[]>([
        "completions",
        date,
      ])

      queryClient.setQueryData<CompletionRow[]>(
        ["completions", date],
        (old) => {
